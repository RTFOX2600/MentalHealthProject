...
def aggregate_canteen_stats(student, start_date, end_date):
    """
    聚合计算食堂消费统计（基于每日统计）
...
def aggregate_gate_stats(student, start_date, end_date):
    """
    聚合计算校门门禁统计（基于每日统计）
...
def aggregate_dormitory_stats(student, start_date, end_date):
    """
    聚合计算寝室门禁统计（基于每日统计）
...
def aggregate_network_stats(student, start_date, end_date):
    """
    聚合计算网络访问统计（基于每日统计）
    ...
    Returns:
        dict: {
            ...
            'night_usage_rate': float,
            'late_night_usage_rate': float,
            'avg_duration': float,
            'max_duration': float
        }
    """
    ...
    for stat in daily_stats:
        ...
        night_rate = stat.statistics_data.get('night_usage_rate', 0)
        late_night_rate = stat.statistics_data.get('late_night_usage_rate', 0)
        ...
        night_duration += daily_avg_duration * (night_rate / 100)
        late_night_duration += daily_avg_duration * (late_night_rate / 100)
...
def aggregate_academic_stats(student, start_date, end_date):
    """
    聚合计算学业成绩统计（基于每日统计）
